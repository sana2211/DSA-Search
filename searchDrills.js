...
function linear(arr, value) {
  ...
  return `Searched ${searchCount} times and item is not in array`;
}

function binary(array, value, start, end, count) {
  var start = start === undefined ? 0 : start;
  var end = end === undefined ? array.length : end;
  var count = count === undefined ? 0 : count;

  if (start > end) {
    return `Searched ${count} times and item is not in array`;
  }
